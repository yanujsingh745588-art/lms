    ...
    <div className='h-screen overflow-scroll flex flex-col items-start justify-between md:p-8 md:pb-0 p-4 pt-8 pb-0'>
      <form onSubmit={handleSubmit} className='flex flex-col gap-4 max-w-md w-full'>
        <div className='flex flex-col gap-1'>
          ...
          className='outline-none md:py-2.5 py-2 px-3 rounded border border-gray-500 bg-white shadow-sm' required/>
        </div>

        <div className='flex flex-col gap-1'>
          <p className='text-lg'>Course Description</p>
          <div className='bg-white shadow-sm border border-gray-500 rounded' ref={editorRef}></div>
        </div>
        ...
        <div className='flex items-center justify-between flex-wrap'>
          ...
          <div className='flex md:flex-row flex-col items-center gap-3'>
            <p className='text-lg'>Course Thumbnail</p>
        ...
        <div className='flex flex-col gap-1'>
          ...
          <input onChange={e=>setDiscount(e.target.value)} type="number" placeholder='0' value={discount} min={0} max={100}
            className='outline-none md:py-2.5 py-2 px-3 rounded border border-gray-500 bg-white shadow-sm' required/>
        </div>
                ...
                <div className='p-4'>
                  {chapter.chapterContent.map((lecture,lectureIndex)=>(
                    <div key={lectureIndex} className='flex justify-between items-center mb-2'>
                     ...
                     <img src={assets.cross_icon} onClick={()=>handleLecture('remove',chapter.chapterId,lectureIndex)} className='cursor-pointer'/>
                    </div>
                  ))}
